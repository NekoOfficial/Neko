 ...
 * And then use its methods anywhere.
...
class Settings {
  ...
  async update(id, obj) {
    ...
    const { value } = await this.client.db.collection(this.collection).findOneAndUpdate(
      { id }, { $set: obj }, { upsert: true, returnOriginal: false, projection: { _id: 0 } }
    );
